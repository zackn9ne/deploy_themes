...
echo sprintf( '<div id="posts-container" class="%sfusion-blog-archive fusion-clearfix" data-pages="%s">', $container_class, $number_of_pages );
	...
	while ( have_posts() ): the_post();
		...
		if( $blog_layout == 'timeline' ) {
			...
			$post_year = get_the_modified_date( 'o' );
			$current_date = get_the_modified_date( 'o-n' );
			...
			if ( $prev_post_month != $post_month ||
				...
				echo sprintf( '<h3 class="fusion-timeline-date">%s</h3>', get_the_modified_date( Avada()->settings->get( 'timeline_date_format' ) ) );
